...
> ⚠️ **اسم الموديل**: `deepseek-chat` القديم **اتشال من قائمة الموديلات يوم
> 2026-07-24**. لسه بيرد كـ alias قديم لحد دلوقتي، بس `models.list()` مابيرجعوش
> خالص — يعني عايش على وقت مستعار. الموديلات المتاحة رسميًا:
> `deepseek-v4-flash` (الرخيص، وهو اللي بنستخدمه) و `deepseek-v4-pro`.
>
> ⚠️ **`deepseek-v4-flash` موديل «تفكير»** — بيطلّع reasoning tokens قبل الإجابة،
> وبتتحسب ضمن الـ output وبتتدفع. عشان كده `AI_MAX_RESPONSE_TOKENS` بـ 900 مش 500:
> قياس على أسئلة حقيقية طلع 178–441 توكن، والسقف القديم كان قريب أوي ولو اتعدى
> الإجابة بتتقطع في نص الكلام.
>
> 💰 **التكلفة الفعلية المقاسة**: حوالي **0.65$ لكل 1000 سؤال** (الملف المعرفي
> بيتبعت مع كل سؤال، ~3900 توكن إدخال). يعني ميزانية 5$ تكفي ~7700 سؤال في الشهر.
>
